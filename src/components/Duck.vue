...
function createVisibleCashPool() {
  ...
  function createFloatingCash() {
    ...
    cash.position.set(
      Math.random() * 0.6 - 0.3,
      Math.random() * 0.1 - 0.07, // Slightly raised for visibility
      Math.random() * 0.4 - 0.2
    );
...
.three-canvas {
  ...
  height: 100vh;
  overflow: hidden;
  background:  rgba(255, 215, 0);  /* Gold */
  position: absolute;
  top: 0;
  left: 0;
}
...
.three-canvas {
  ...
  height: 100vh;
  overflow: hidden;
  background: url('/3d-bear-arts/assets/bg.jpg'); /* Replace with the correct image path */
